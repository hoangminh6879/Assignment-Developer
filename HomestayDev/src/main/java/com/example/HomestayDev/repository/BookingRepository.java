...
public interface BookingRepository extends JpaRepository<Booking, UUID> {
    ...
    @Query("SELECT h.name, COUNT(b), SUM(b.totalPrice) FROM Booking b JOIN b.homestay h WHERE b.paymentStatus = 'PAID' GROUP BY h.id, h.name ORDER BY SUM(b.totalPrice) DESC")
    List<Object[]> getTopHomestaysAdmin();

    @Query("SELECT SUM(b.totalPrice) FROM Booking b WHERE b.homestay.host.username = :username AND b.paymentStatus = 'PAID'")
    ...
    @Query("SELECT h.name, COUNT(b), SUM(b.totalPrice) FROM Booking b JOIN b.homestay h WHERE h.host.username = :username AND b.paymentStatus = 'PAID' GROUP BY h.id, h.name")
    List<Object[]> getHomestayStatsHost(@Param("username") String username);

    @Query("SELECT SUBSTRING(CAST(b.createdAt AS string), 1, 7) as m, SUM(b.totalPrice) FROM Booking b WHERE b.paymentStatus = 'PAID' GROUP BY m ORDER BY m ASC")
    List<Object[]> getMonthlyRevenueAdmin();

    @Query("SELECT SUBSTRING(CAST(b.createdAt AS string), 1, 7) as m, SUM(b.totalPrice) FROM Booking b WHERE b.homestay.host.username = :username AND b.paymentStatus = 'PAID' GROUP BY m ORDER BY m ASC")
